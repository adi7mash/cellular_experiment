...
Do pairwise molecular interaction representations encode system-level biology? We show that frozen embeddings from a self-supervised model, trained only on protein-ligand sequence pairs, capture cellular phenotype, transcriptomic response, and biological network structure — without any cellular data. On RxRx3, our compound embeddings achieve 81.1% CC AUROC (pair-level CV) vs. 76.0% for Boltz-2, and 60.3% zero-shot per-target AUROC vs. 53.9% — with a trained model reaching 89.9% on held-out genes. On Tahoe-100M pseudobulk transcriptomics, zero-shot CG profiles — 945 binding scores from the frozen checkpoint — predict drug-induced gene expression changes at 0.929 Pearson delta correlation across 2,847 conditions and 8 cell lines, competitive with MAP (Arc Institute), which uses a 600M-parameter cell encoder, a 187K-drug knowledge graph, and 4TB of training data. On JUMP-CP (EFAAR framework, 7,976 genes), our protein embeddings recover protein complexes, pathways, and signaling networks, exceeding the best Cell Painting model (MAE-G/8, 93M cell images) by 20–59% across all sources. These results demonstrate multi-modal emergence: the same frozen checkpoint predicts what molecules do to cells (morphology and transcriptomics) and captures how proteins organize within cells (biological networks) — across three independent benchmarks, three data modalities, and three evaluation frameworks. Pair-level self-supervision encodes enough mechanistic structure for cellular-level reasoning to arise from composition alone.
...
We present evidence that it does — from three independent benchmarks across three data modalities. First, we show that frozen compound embeddings predict what molecules do to living cells: on the RxRx3 phenomics benchmark, they exceed Boltz-2's structural pipeline (81.1% vs. 76.0% CC AUROC) while a trained per-target model reaches 89.9% on held-out gene targets. Second, on Tahoe-100M pseudobulk transcriptomics, zero-shot CG profiles predict drug-induced gene expression changes at 0.929 Pearson delta correlation, competitive with MAP (Arc Institute) — a purpose-built perturbation model using a 600M-parameter cell encoder and a 187K-drug knowledge graph. Third, frozen protein embeddings from the same checkpoint capture the functional organization of biology: on the JUMP-CP known-relationship benchmark (EFAAR framework, 7,976 genes), they recover protein complexes, metabolic pathways, and signaling networks better than the best published Cell Painting model trained on 93 million cell images (Kraus et al., 2025). In all cases, the model has never seen a cell, a microscopy image, or any cellular data.

These three findings are complementary. The compound embeddings demonstrate that pair-level representations predict *perturbation responses* — what happens when you add a molecule to a cell — across two independent readouts (morphology and transcriptomics). The protein embeddings demonstrate that the same representations capture the *wiring diagram* — which proteins work together in complexes, share pathways, and participate in signaling cascades. Together, they provide evidence that pair-level self-supervision encodes both the mechanism of molecular action and the organizational structure of cellular biology.

This connects to the broader goal of building a "virtual cell." Current approaches are either top-down (training directly on cellular readouts) or bottom-up (explicitly simulating molecular interactions). We present evidence for a third path: learning interaction representations rich enough that cellular behavior emerges from their composition — across morphology, transcriptomics, and network biology — without explicit simulation or cellular training data.
...
1. **Multi-modal evidence for emergent biological reasoning**: Frozen pairwise interaction embeddings encode cellular phenotype (RxRx3, morphology), transcriptomic perturbation response (Tahoe-100M), and biological network structure (JUMP-CP) — three qualitatively different levels of system-level biology, none present in the training signal.

2. **Cross-dataset, cross-modality validation**: Results hold across RxRx3 (1,674 compounds, 735 genes), Tahoe-100M (2,847 conditions, 8 cell lines), and JUMP-CP (7,976 genes), against published baselines from the structural biology (Boltz-2), perturbation modeling (MAP), and Cell Painting (MAE-G/8) communities, on each community's own benchmark framework.
...
**Perturbation response prediction.** Predicting how drugs alter gene expression is a central challenge in computational biology. CPA (Lotfollahi et al., 2023) uses compositional perturbation autoencoders. GEARS (Roohani et al., 2023) leverages gene regulatory graphs. MAP (Zhong et al., 2026) combines a frozen 600M-parameter cell encoder (STATE SE-600M) with a LLaMA-style perturbation encoder and a 187K-drug knowledge graph, achieving ~0.90 Pearson on seen drug-cell combinations and ~0.87 on unprofiled drugs. These models require large-scale single-cell training data and purpose-built architectures. We show that a 945-dimensional zero-shot binding vector from a frozen checkpoint achieves competitive performance (0.929 Pearson) with a simple MLP projection.
...
### 3.3 Tahoe-100M Transcriptomics Benchmark

Tahoe-100M (Tahoebio, 2025) is a single-cell transcriptomics atlas of chemical perturbations: 95.6 million cells across 50 cell lines treated with 379 drugs at 3 concentrations. Each cell profile records expression values for 51,088 genes. We evaluate on pseudobulk aggregates — mean expression per (drug, cell_line) condition — using expression delta (perturbed minus DMSO control) as the prediction target.

**CG profile.** For each drug, we compute a 945-dimensional CG profile: the vector of dot products between the drug's molecule codebook token (1,280-dim) and each of 945 protein codebook tokens (1,280-dim), all from the frozen checkpoint. This profile encodes the drug's predicted binding pattern across the human proteome — a zero-shot representation requiring no cellular data.

**Evaluation metric.** Pearson delta correlation: the Pearson correlation between predicted and actual log fold-changes across the 2,000 highest-variance genes (HVGs), averaged across all conditions. This is the standard metric for perturbation prediction models (Lotfollahi et al., 2023; Bunne et al., 2024).

**Comparison.** MAP (Arc Institute, Nature Machine Intelligence, Aug 2026) uses a frozen 600M-parameter STATE cell encoder, a LLaMA-style perturbation encoder, a 187K-drug knowledge graph encoder, and 4TB of single-cell training data. MAP reports ~0.90 Pearson on seen drug-cell combinations and ~0.87 on "unprofiled" drugs (5% held-out per cell line, not full leave-one-drug-out).

### 3.4 Embedding Source
...
Under pair-level 5-fold CV on 246 compounds and 735 gene targets, our frozen embeddings significantly outperform Beaini et al.'s Boltz-2 pipeline (Table 1).
...
### 5.12 Transcriptomics: Zero-Shot CG Profiles Predict Gene Expression Changes

We tested whether CG profiles — 945 binding scores from the frozen checkpoint — predict drug-induced gene expression changes measured by single-cell RNA sequencing. This is a qualitatively different readout from Cell Painting morphology: transcriptomics measures the molecular state of cells directly, providing ground truth at the gene level.

**Data.** We computed pseudobulk expression profiles from Tahoe-100M by averaging across cells per (drug, cell_line) condition, sampling 300 of 3,388 shards (1.7% of raw cells, evenly spaced for coverage). This yielded 3,024 conditions across 8 cell lines and 51,088 genes. After matching to compounds with CG profiles and filtering (≥10 cells per condition): 2,847 valid conditions, 357 drugs, 8 cell lines. Target: expression delta (perturbed − DMSO control) over the 2,000 highest-variance genes.

**Setup.** CG profiles are frozen and zero-shot. The only trained component is a projection MLP mapping the 945-dim CG profile (optionally concatenated with an 8-dim cell-line one-hot) to 2,000-dim gene expression delta. 5-fold cross-validation on conditions.

**Table 11.** Tahoe-100M pseudobulk transcriptomics prediction (Pearson delta correlation, 5-fold CV).

| Method | Input | Pearson Δ | DEG-50 |
|--------|-------|-----------|--------|
| Ridge regression | CG profile (945d) | 0.822 | — |
| MLP [2048,1024] | CG profile (945d) | 0.866 | 0.881 |
| MLP [2048,1024] | CG + one_hot (953d) | 0.923 | 0.933 |
| **MLP [4096,2048,1024]** | **CG + one_hot (953d)** | **0.929** | **0.937** |
| MLP [4096,2048,1024] | CG + expr_945 (1890d) | 0.917 | 0.926 |

CG profiles achieve 0.866 Pearson delta correlation without any cell-line information — a zero-shot binding representation predicting gene expression changes across 2,000 genes. Adding an 8-dim cell-line identifier lifts performance to 0.929, demonstrating that the CG profile captures the drug's mechanism while the cell-line encoding captures the cellular context. The DEG-50 metric (Pearson on the 50 most differentially expressed genes per condition) reaches 0.937, indicating particularly strong prediction of the most biologically meaningful expression changes.

**Comparison to MAP.** MAP (Arc Institute) reports ~0.90 Pearson on seen drug-cell combinations using a 600M-parameter frozen cell encoder (STATE SE-600M), a LLaMA-style perturbation encoder, a 187K-drug knowledge graph encoder, and 4TB of single-cell training data. Our 0.929 uses a 945-dimensional zero-shot binding vector and a 3-layer MLP. The CG profile replaces the 600M-parameter cell encoder, the knowledge graph, and the perturbation encoder with 945 dot products from a frozen checkpoint.

MAP's "unprofiled drug" evaluation holds out 5% of drugs per cell line and reports ~0.87; our 5-fold CV randomly splits conditions, making the results most comparable to MAP's "seen combination" setting (~0.90). Our LODO (leave-one-drug-out) baseline is 0.221 — substantially harder, as it requires generalizing from one drug's CG→expression mapping to another's without chemical structural similarity information.

**Cell conditioning from Bonbon's proteome.** The 945-protein expression profile (expression of the same 945 proteins in the CG profile, extracted from DMSO controls) achieves 0.917, recovering 81% of the one_hot's lift over CG-only: (0.917 − 0.866) / (0.929 − 0.866) = 81%. This demonstrates that Bonbon's own proteome — the 945 proteins whose binding patterns define the CG profile — doubles as a cell-state representation. The remaining gap is structural: with 8 unique cell lines at pseudobulk resolution, there are only 8 distinct expression states, and an 8-dim one-hot is information-theoretically optimal for encoding 8 categories. At single-cell resolution, where thousands of unique cellular states exist, expression conditioning becomes essential.

**Data efficiency.** With only 5% of training data (~142 conditions), CG profiles achieve 0.509 Pearson — well above chance, and notably strong given only ~14 drugs per fold. At 10% (~285 conditions), performance reaches 0.654 with cell-line conditioning. The frozen CG representation provides a strong prior that enables learning from small perturbation datasets.

## 6. Discussion

### 6.1 Multi-Modal Emergence: Pharmacology, Transcriptomics, and Cellular Organization

The model was trained on protein-ligand sequence pairs. It was never exposed to cellular data, microscopy images, phenomics readouts, transcriptomics, or any system-level biological signal. Yet its frozen representations exhibit emergence at three distinct levels.
...
**Transcriptomic emergence (gene expression).** The same 945-dimensional CG profiles that encode binding patterns predict drug-induced gene expression changes at 0.929 Pearson delta correlation — competitive with purpose-built perturbation models using 600M-parameter cell encoders and 187K-drug knowledge graphs. The binding representation, never trained on expression data, encodes enough about how drugs perturb cellular gene programs to predict transcriptomic response across 2,000 genes.
...
The combination of three independent findings is substantially stronger than any individual result. The model has learned the *perturbation response function* across two independent readouts (morphology and transcriptomics), *and* the *biological network structure* (how proteins organize into functional units). Neither was in the training data. In language model terms, this is not just arithmetic emerging from next-token prediction — it is the model learning syntax, semantics, and pragmatics: the operations, the structure they operate on, and the downstream consequences.
...
**Our work demonstrates a third path: bottom-up via learned representations.** A self-supervised model trained only on protein-ligand sequence pairs learns interaction representations that provide both essential ingredients for a virtual cell: the *perturbation response function* — compound embeddings predict both cellular morphology (Section 5.1-5.3) and transcriptomic response (Section 5.12) — and the *wiring diagram* (protein embeddings recover the functional organization of protein complexes, pathways, and signaling networks, Section 5.9). Neither was in the training data, and no cellular data, structural simulation, or hand-engineered aggregation was required.

The transcriptomics result (Section 5.12) strengthens this claim substantially. Predicting gene expression changes is a more mechanistically grounded readout than morphological similarity: it measures the molecular state of cells directly, connecting drug binding patterns to specific gene programs. That 945 binding scores predict 2,000-gene expression deltas at 0.929 Pearson — competitive with a 600M-parameter model purpose-built for this task — suggests the CG profile encodes not just which proteins a drug binds, but the downstream consequences of that binding across the transcriptome.

The protein-side result (Section 5.9) completes the picture. Our embeddings capture which proteins work together — in complexes (CORUM, HuMAP), metabolic pathways (Reactome), and signaling cascades (SIGNOR) — better than models trained on millions of cell images. The compound embeddings predict *what happens* when you perturb this network; the protein embeddings capture *the network itself*.
...
We have demonstrated that frozen embeddings from a self-supervised molecular interaction model encode system-level biology from three independent perspectives. Compound embeddings predict cellular phenotype (81.1% CC AUROC, 89.9% per-target on held-out genes) and transcriptomic perturbation response (0.929 Pearson delta correlation, competitive with purpose-built 600M-parameter models). Protein embeddings from the same frozen checkpoint recover the functional organization of biology — protein complexes, metabolic pathways, and signaling networks — exceeding Cell Painting models trained on 93 million cell images. No form of system-level knowledge was present in the training signal.

These results hold across three datasets (RxRx3, Tahoe-100M, JUMP-CP), three data modalities (morphology, transcriptomics, network biology), against published baselines from the structural biology (Boltz-2), perturbation modeling (MAP), and Cell Painting (MAE-G/8) communities, on each community's own benchmark framework. We introduce a rigorous evaluation protocol for molecular phenomics prediction that distinguishes inflated from honest performance measures.

The multi-modal emergence — pharmacological on the compound side, transcriptomic through CG profiles, organizational on the protein side — suggests the codebook has learned an implicit biological ontology: a compressed representation of how molecules act and how proteins organize, entirely from protein-ligand sequence pairs. The same frozen checkpoint, the same self-supervised pretraining, produces representations that compose from pairwise interactions to system-level structure at every level we have tested: binding sites, mechanisms of action, cellular morphology, gene expression programs, protein complexes, pathways, and signaling networks.
...
- Bunne, C., et al. (2024). How to build the virtual cell with artificial intelligence: priorities and opportunities. Cell.
...
- Tahoebio. (2025). Tahoe-100M: 95.6M single-cell transcriptomic profiles of chemical perturbations.
- Zhong, E. D., et al. (2026). MAP: Modeling perturbation responses with masked autoregressive perturbation prediction. Nature Machine Intelligence.
